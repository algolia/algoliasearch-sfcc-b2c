...
describe('updateCPObjectFromXML', () => {
    describe('price book extraction', () => {
        test('reads the price-table product-ids', () => {
            ...
            const result = jobHelper.updateCPObjectFromXML(xmlFile(PRICEBOOK_ONE_VARIANT), changedProducts, 'pricebook');
    ...
    describe('inventory list extraction', () => {
        test('reads the record product-ids', () => {
            const changedProducts = [];
            const result = jobHelper.updateCPObjectFromXML(xmlFile(INVENTORY_ONE_VARIANT), changedProducts, 'inventory');
    ...
    describe('error handling', () => {
        test('returns success=false with an error message when reading throws', () => {
            const badFile = {
                ...
                // no __xmlContent -> FileReader yields '' -> reader produces no terminator, but
                // to force a throw we make exists() succeed and content access blow up:
