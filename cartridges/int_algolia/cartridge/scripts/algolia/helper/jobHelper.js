...
/**
 * Normalizes a product ID read from a price book or inventory delta to the set of IDs that correspond
 * to actual indexed records under the current record model, so the consumer rebuilds the records a full
 * index would hold rather than the raw object the delta happened to name.
 *
 * The delta names the object whose price-table or inventory record changed: usually a variant SKU, but
 * a master when a merchant edits a master-level price-table that variants inherit. The record level the
 * model indexes at decides how that ID maps to records:
 * - 'master' (master-level and attribute-sliced record models, or variant-level with attributes computed
 *   from the base product): the indexed record is the master, so a changed variant is rolled up to its
 *   master; masters and standalone products pass through.
 * - 'variant' (plain variant-level): the indexed records are the variants, so a changed master is fanned
 *   out to its variants; variants and standalone products pass through.
 * An ID that resolves to no product passes through so the consumer can issue a delete (a no-op if the
 * record is absent).
 *
 ...
 * @returns {string[]} the IDs to record as changed for this entry
 */
function _resolveToIndexedRecordIDs(productID, recordLevel) {
 ...
 *        indexed records (see `_resolveToIndexedRecordIDs`): 'master' rolls variants up to their master,
                    ...
                    while (xmlStreamReader.hasNext()) {
                        ...
                        if (xmlEvent === XMLStreamConstants.START_ELEMENT) {
                            ...
                            if (xmlStreamReader.getLocalName() === 'record') { // <record> start element
                                ...
                                let resolvedIDs = _resolveToIndexedRecordIDs(productID, recordLevel);
