...
//
//                      ID                        ║                      description                         ║ SFCC preference data type
//   ═════════════════════════════════════════════╬══════════════════════════════════════════════════════════╬════════════════════
...
//   IndexOutOfStock                              ║ Whether out-of-stock products should be indexed          ║ boolean
//   EnableRealTimeInventoryHook                  ║ Sends inventory updates to Algolia after each order      ║ boolean
...
//   RecordModel                                  ║ Shape of the records generated by the cartridge          ║ enum-of-string
//   AttributeSlicedRecordModel_GroupingAttribute ║ Grouping attribute for the Attribute-sliced record model ║ string
//   IndexingAPI                                  ║ The indexing API to use (Search or Ingestion)            ║ string
//   AnalyticsRegion                              ║ Required for Ingestion, can be "us" or "eu"              ║ string
//   LocalesForIndexing                           ║ Locales to index                                         ║ set-of-string
...
//   EnablePricingLazyLoad                        ║ If enabled, prices are fetched on search result display  ║ boolean
//  ══════════════════════════════════════════════╩══════════════════════════════════════════════════════════╩════════════════════
...
function getPreference(id) {
    ...
    if (value === null) {
        return '';
    } else if (typeof value === 'object' && 'getValue' in value) {
...
/**
 * @description Getting preference (as an array) for Algolia
 * @param {string} id name of preference
...
/**
 * Retrieves all site preferences with the "Algolia_" prefix.
 * Note that AdminApiKey is filtered out and should not be re-added for privacy reasons.
 * Can be used for logging and debugging purposes.
 * @returns {Object} an object with key-value pairs (sitePrefName: sitePrefValue), can be stringified
 */
function getAlgoliaSitePreferences() {
    ...
    for (let i = 0; i < algoliaPreferenceKeys.length; i++) {
        let sitePrefKeyWithPrefix = algoliaPreferenceKeys[i];
...
module.exports = {
    ...
    getAlgoliaSitePreferences: getAlgoliaSitePreferences,
};
